...
class LeNetConvPoolLayer(object):
    ...
    def __init__(self, rng, input, filter_shape, image_shape, poolsize=(2, 2), hand_picked=False,
                 pool_ignore_border=True, d_gauss_filter=None):
        """
        ...
        if hand_picked:
            self.W = theano.shared(
            numpy.asarray(
                d_gauss_filter(rng.uniform(low=-W_bound, high=W_bound, size=filter_shape), 10*-4),
                dtype=theano.config.floatX
            ),
            borrow=True
        )
